...
public class Helper {
	...
	public static void initializeStaticFields(FieldInfo[] staticFields, ClassInfo ci,
	  ...
	  public static int addNewHeapNode(
		  ...
		  String refChain = ((SymbolicInteger) attr).getName(); //TODO + "{" + daIndex + "}"; // do we really need to add daIndex here?
		  SymbolicInteger newSymRef = (SymbolicInteger) attr;// TODO was new SymbolicInteger( refChain, uniqueSymId);
          ...
          if (typeClassInfo.isArray()) {
              ...
              PCChoiceGenerator temp_cg = ti.getVM().getLastChoiceGeneratorOfType(PCChoiceGenerator.class);
              // 24-04-2020 Added assert statement: Hendrik Winkelmann
              assert temp_cg != null;
              temp_cg.getCurrentPC().arrayExpressions.put(eiRef.toString(), arrayAttr);
          }
		  ...
		  symInputHeap._add(n);
		  //pcHeap._addDet(Comparator.NE, newSymRef, new IntegerConstant(-1)); // Hendrik Winkelmann: TODO Necessary? numSymRefs should be > 0.
		  // 28-04-2020 Added name for constant for easier debugging: Hendrik Winkelmann
		  pcHeap._addDet(Comparator.EQ, newSymRef, new IntegerConstant(numSymRefs, typeClassInfo.getSimpleName()));
		  for (int i=0; i< numSymRefs; i++)
			  pcHeap._addDet(Comparator.NE, n.getSymbolic(), prevSymRefs[i].getSymbolic());
		  return daIndex;
	  }

	  // 24-04-2020 Reformatted: Hendrik Winkelmann
	  public static HelperResult addNewArrayHeapNode(
		  ...
		  int daIndex = ti.getHeap().newObject(typeClassInfo, ti).getObjectRef();
		  ti.getHeap().registerPinDown(daIndex);
		  	// Perhaps, this can only be re-added if for arrays this distinction with daIndex is met at different points in the code? : Hendrik Winkelmann
		  String refChain = ((ArrayExpression) attr).getName();//TODO + "{" + daIndex + "}"; // do we really need to add daIndex here?
		  SymbolicInteger newSymRef = (SymbolicInteger) attr;// TODO was new SymbolicInteger(refChain); : Hendrik Winkelmann
		  ElementInfo eiRef =  ti.getModifiableElementInfo(daIndex);//ti.getElementInfo(daIndex); // TODO to review!
		  if(setShared) {
			  eiRef.setShared(ti,true);//??
		  }
