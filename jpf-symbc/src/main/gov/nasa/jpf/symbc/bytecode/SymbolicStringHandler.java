...
public class SymbolicStringHandler {
	...
	public boolean isMethodStringSymbolic(JVMInvokeInstruction invInst, ThreadInfo th) {
		// 26-06-2020: This does not seem to be thought out. I replaced it to fit my use-case: Hendrik Winkelmann
		// Only treat equal-methods with two Strings of which at least one String is symbolic as a symbolic string-method:
		if (invInst.getArgSize() == 2 && invInst.getInvokedMethod().getName().equals("equals")) {
			StackFrame sf = th.getModifiableTopFrame();
			if (sf.getOperandAttr(0) instanceof StringSymbolic || sf.getOperandAttr(1) instanceof StringSymbolic) {
				Heap heap = th.getHeap();
				ElementInfo e1 = heap.get(sf.peek(0));
				ElementInfo e2 = heap.get(sf.peek(1));
				return e1 != null && e2 != null && e1.isStringObject() && e2.isStringObject();
			}
		}
		return false;
	}
	...
	public Instruction handleSymbolicStrings(JVMInvokeInstruction invInst, ThreadInfo th) {
		...
		if (needToHandle) {
			...
			if (shortName.equals("concat")) {
				...
				handlebooleanValue(invInst, th); // 29-06-2020 Changed from handlebooleanValue: Hendrik Winkelmann
			} else {
	...
	// Changed method's signature : Hendrik Winkelmann
