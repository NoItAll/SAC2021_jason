...
public class AALOAD extends gov.nasa.jpf.jvm.bytecode.AALOAD {
  ...
  public Instruction execute (ThreadInfo ti) {
      ...
      if (!ti.isFirstStepInsn()) {
          ...
          ((PCChoiceGenerator)cg).setOffset(this.position);
          ((PCChoiceGenerator)cg).setMethodName(this.getMethodInfo().getFullName());
          ti.getVM().setNextChoiceGenerator(cg); // This will not delete the previously set cg, rather, it is cascaded.
          return this;
      } else {
    	  // Retrieve the choice generators from the prior step
          cg = ti.getVM().getLastChoiceGeneratorOfType(PCChoiceGenerator.class);
      ...
      // Do this solely to get the last pathcondition which is to be extended
      PathCondition pc;
      ChoiceGenerator<?> prev_cg = cg.getPreviousChoiceGeneratorOfType(PCChoiceGenerator.class);
      ...
      if (prev_cg == null)
          ...
          pc = ((PCChoiceGenerator)prev_cg).getCurrentPC(); // PC is COPIED
      ...
      if (currentChoice == 1) {
        ...
        if (pc.simplify()) { // satisfiable
          ...
          pc._addDet(Comparator.LT, indexAttr, arrayAttr.length);		 // Must be in bounds
          pc._addDet(Comparator.GE, indexAttr, new IntegerConstant(0)); // Must be in bounds
          ...
          if (prevHeapCG == null) {
              ...
              // 23-04-2020 Added prevSymRefs and numSymRefs assignation as elsewise they would always
              // be null and 0 correspondingly: Hendrik Winkelmann
              prevSymRefs = symInputHeap.getArrayNodesOfType(typeClassInfo, arrayRef);
              numSymRefs = prevSymRefs.length;
          }
