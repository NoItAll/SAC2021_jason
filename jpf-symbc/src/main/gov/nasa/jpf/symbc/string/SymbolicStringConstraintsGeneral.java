...
public class SymbolicStringConstraintsGeneral {
  ...
  static Logger logger = LogManager.getLogger("stringsolver");
	...
	public static final String DUMMY_EQUALS = "DEQ"; // Introduced dummy solver: Hendrik Winkelmann
	...
	// Added this dummy implementation for rudimentary solving symbolic strings with String.equals(...) : Hendrik Winkelmann
	private boolean solveEqAndNeq(StringPathCondition pc) {
		Map<StringSymbolic, String> equalsTo = new HashMap<>();
		Map<StringSymbolic, List<String>> notEqualsTo = new HashMap<>();
		pc.solution = new HashMap<>();
		reset(pc);
		StringConstraint current = pc.header;
		// Check relation for all StringSymbolics in the path condition
		while (current != null) {
			// Only EQUALS && NOTEQUALS are supported
			if (current.comp == StringComparator.EQUALS) {
				// Check if both are symbolic, constant, or if only one is symbolic
				if (current.left instanceof StringSymbolic && current.right instanceof StringSymbolic) {
					StringSymbolic left = (StringSymbolic) current.left;
					StringSymbolic right = (StringSymbolic) current.right;
					// Check if at least on of them is already solved and thus fixed to a concrete value
					if (!left.solution.equals(StringSymbolic.UNDEFINED) && 
							!right.solution.equals(StringSymbolic.UNDEFINED)) {
						if (!left.solution.equals(right.solution)) {
							return false;
						}
					} else if (!left.solution.equals(StringSymbolic.UNDEFINED)) {
						if (!equalsTo(right, left.solution, equalsTo, notEqualsTo)) {
							return false;
						}
					} else if (!right.solution.equals(StringSymbolic.UNDEFINED)) {
						if (!equalsTo(left, right.solution, equalsTo, notEqualsTo)) {
							return false;
						}
					} else {
						return false; // These more complex relations (between two symbolic, non-fixed strings) are not handled by this dummy
					}
				} else if (current.left instanceof StringConstant && current.right instanceof StringConstant){
					StringConstant leftConstant = (StringConstant) current.left;
					StringConstant rightConstant = (StringConstant) current.right;
					if (!leftConstant.value.equals(rightConstant.value)) {
						return false;
					}
				} else {
					// One is constant, the other is symbolic
					StringConstant constant = (StringConstant) 
							(current.left instanceof StringConstant ? current.left : current.right);
					StringSymbolic symbolic = 
							(StringSymbolic) (current.left instanceof StringSymbolic ? current.left : current.right);
					if (!equalsTo(symbolic, constant.value, equalsTo, notEqualsTo)) {
						return false;
					}
				}
			} else if (current.comp == StringComparator.NOTEQUALS) {
				if (current.left instanceof StringSymbolic && current.right instanceof StringSymbolic) {
					StringSymbolic left = (StringSymbolic) current.left;
					StringSymbolic right = (StringSymbolic) current.right;
					if (!left.solution.equals(StringSymbolic.UNDEFINED) && 
						...
						if (!notEqualsTo(right, left.solution, equalsTo, notEqualsTo)) {
							return false;
						}
					} else if (!right.solution.equals(StringSymbolic.UNDEFINED)) {
						if (!notEqualsTo(left, right.solution, equalsTo, notEqualsTo)) {
							return false;
						}
					} else {
						return false; // These more complex relations (between two symbolic, non-fixed strings) are not handled by this dummy
					...
					StringSymbolic symbolic = 
							(StringSymbolic) (current.left instanceof StringSymbolic ? current.left : current.right);
					if (!notEqualsTo(symbolic, constant.value, equalsTo, notEqualsTo)) {
						return false;
					}
				}
			} else {
		...
		for (Map.Entry<StringSymbolic, String> entry : equalsTo.entrySet()) {
			entry.getKey().solution = entry.getValue();
			pc.solution.put(entry.getKey().getName(), entry.getValue());
		}
		return true;
	}
	
	// Added this for experimentation : Hendrik Winkelmann
	private void reset(StringPathCondition sc) {
		StringConstraint temp = sc.header;
		// Reset all current solutions. 
		while (temp != null) {
			if (temp.left instanceof StringSymbolic) {
				((StringSymbolic) temp.left).solution = StringSymbolic.UNDEFINED;
			}
			if (temp.right instanceof StringSymbolic) {
				((StringSymbolic) temp.right).solution = StringSymbolic.UNDEFINED;
			}
			temp = temp.and;
		}
	}
	
	// Added this for the dummy implementation for rudimentary solving symbolic strings with String.equals(...) : Hendrik Winkelmann
	// Checks if it is possible for sym to be equal to constantVal.
	// Enhances the equalsTo-map correspondingly.
	protected boolean equalsTo(
			StringSymbolic sym,
			String constantVal,
			Map<StringSymbolic, String> equalsTo,
			Map<StringSymbolic, List<String>> notEqualsTo) {
		List<String> notEqualsToSym = notEqualsTo.get(sym);
		if (notEqualsToSym != null && notEqualsToSym.contains(constantVal)) {
			return false;
		}
		
		equalsTo.put(sym, constantVal);
		
		return true;
	}
	
	// Added this for the dummy implementation for rudimentary solving symbolic strings with String.equals(...) : Hendrik Winkelmann
	// Checks if it is possible for sym to be not equal to constantVal.
	// Enhances the notEqualsTo-map correspondingly.
	protected boolean notEqualsTo(
			StringSymbolic sym, 
			String constantVal, 
			Map<StringSymbolic, String> equalsTo, 
			Map<StringSymbolic, List<String>> notEqualsTo) {
		String equalsToSym = equalsTo.get(sym);
		if (equalsToSym != null && equalsToSym.equals(constantVal)) {
			return false;
		}
		
		List<String> notEqualsToSym = notEqualsTo.get(sym);
		if (notEqualsToSym == null) {
			notEqualsToSym = new ArrayList<>();
			notEqualsTo.put(sym, notEqualsToSym);
		}
		if (!notEqualsToSym.contains(constantVal)) {
			notEqualsToSym.add(constantVal);
		}
	...
	private boolean inner_isSatisfiable(StringPathCondition pc) {
		...
		/* Set up solver */
		// Added the dummy solver: Hendrik Winkelmann
		if (string_dp[0].equalsIgnoreCase("deq")) {
			solver = DUMMY_EQUALS;
			return solveEqAndNeq(pc);
		}
