...
public class JPF_gov_nasa_jpf_symbc_Debug extends NativePeer {
    ...
    // 15-05-2020 Added: Hendrik Winkelmann
    @MJI
    ...
    // 22-05-2020 Added: Hendrik Winkelmann
    @MJI
    public static boolean isSymbolicBoolean(MJIEnv env, int objRef, boolean b) {
    	Object[] attrs = env.getArgAttributes();
    	if (attrs != null) {
    		IntegerExpression sym_arg = (IntegerExpression) attrs[0];
    		if (sym_arg != null) {
    ...
    public static int getSymbolicRealValue(MJIEnv env, int objRef, double v) {
        ...
        RealExpression sym_arg = (RealExpression) attrs[0];
        if (sym_arg != null)
            return env.newString(""+sym_arg.solution());
        else
