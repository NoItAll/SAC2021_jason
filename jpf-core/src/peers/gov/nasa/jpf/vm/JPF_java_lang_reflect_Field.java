...
public class JPF_java_lang_reflect_Field extends NativePeer {
  ...
  public int get__Ljava_lang_Object_2__Ljava_lang_Object_2 (MJIEnv env, int objRef, int ownerRef) {
    ...
    // 15-05-2020 Added support of attributes for symbolic variables + refactored: Hendrik Winkelmann
    int ref;
    ...
    if (!(fi instanceof ReferenceFieldInfo)) { // primitive type, we need to box it
      ...
      setAttrsForRef(ref, attr, env);
      return ref;
      
    } else { // it's a reference
      ref = ei.getReferenceField(fi); // we internally store it as int
      return ref;
    }
  }

  // 15-05-2020 Added to set attributes during get__Ljava_lang_Object_2__Ljava_lang_Object_2(...)
  // To make it more efficient, probably the ElementInfo should be directly derived from MJIEnv TODO: Hendrik Winkelmann
  private void setAttrsForRef(int ref, Object attr, MJIEnv env) {
	  if (attr != null) {
		ElementInfo ei = env.getHeap().get(ref);
		assert ei != null;
	  	FieldInfo fi = ei.getFieldInfo("value"); // Symbolic values are interesting for wrapper objects
		assert fi != null;
  ...
  protected static boolean setValue(MJIEnv env, FieldInfo fi, int obj, int value, Object attr) {
    ...
    if (tci.isPrimitive()) {
      ...
      // 28-04-2020 To also account for reflection during symbolic execution, I added the check for the
      // given value-field of the to-be-set argument: Hendrik Winkelmann
      // TODO Should probably be tested more thoroughly, as I do not know what attr usually is in the
      	// concrete case.
      ElementInfo valueInfo = env.getModifiableElementInfo(value);
      attr = attr == null ? env.getModifiableElementInfo(value)
    		  .getFieldAttr(valueInfo
    				  .getFieldInfo("value")) : attr;
