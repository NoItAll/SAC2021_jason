...
public class CoverageAnalyzer extends ListenerAdapter implements PublisherExtension {
  ...
  // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
  ...
  public static class MethodCoverage {
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public Coverage getCoveredInsn() {
      int nTotal = mi.getInstructions().length;
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public Coverage getCoveredLines() {
 ...
 // 08-05-2020 Added for use in delegating listener: Hendrik Winkelmann
    public BitSet[] getCoveredLinesBS() {
    	BitSet executable = new BitSet();
        BitSet covered = new BitSet();

        getCoveredLines(executable, covered);
        return new BitSet[] {executable, covered};
    }
    
    // 08-05-2020 Copied from getCoveredLines(...) and adapted to only return executable lines: Hendrik Winkelmann
    public BitSet getExecutableLinesAsBS() {
    ...
    // 08-05-2020 Copied and adapted from getCoveredLines(...) and adapted to only return covered lines: Hendrik Winkelmann
    // TODO Refactor
    public BitSet getCoveredLinesAsBS() {
        Instruction inst[] = mi.getInstructions();
        BitSet insn;
        BitSet covered = new BitSet();
        int line;
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public boolean getCoveredLines(BitSet executable, BitSet covered) {
      Instruction inst[] = mi.getInstructions();
      BitSet insn;
      int i, line;
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public Coverage getCoveredBranches() {
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public BitSet getHandlerStarts() {
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public Coverage getCoveredBasicBlocks() {
      BitSet bExec = getExecutedInsn();
      BitSet bb = getBasicBlocks();
      int nCov = 0;
  ...
  public static class ClassCoverage {
    ...
    // 08-05-2020 Added getter for methods-coverages for use in delegating listener: Hendrik Winkelmann
    public Map<MethodInfo, MethodCoverage> getMethodCoverages() {
    	return methods;
    }
    ...
    // 08-05-2020 Added for use in delegating listener: Hendrik Winkelmann
    public void resetMethodCoverage(MethodInfo mi) {
    	MethodCoverage mc = new MethodCoverage(mi);
    	methods.put(mi, mc);
    }
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public MethodCoverage getMethodCoverage(MethodInfo mi) {
      if (methods == null) {
        setLoaded(ci);
      }
    ...
    // 08-05-2020 Changed visibility to public for use in delegating listener: Hendrik Winkelmann
    public Coverage getCoveredMethods() {
    ...
    // 08-05-2020 Added to use in delegating listener: Hendrik Winkelmann
    public BitSet getCoveredLinesAsBS() {
    	BitSet result = new BitSet();
    	
    	for (MethodCoverage mc : methods.values()) {
    		result.or(mc.getCoveredLinesAsBS());
    	}
    	
    	return result;
    }
    
    public void resetMethodCoverages() {
    	for (MethodInfo mi : methods.keySet()) {
    		resetMethodCoverage(mi);
    	}
    }
  ...
  // 08-05-2020 Added getter for class-coverages for use in delegating listeners: Hendrik Winkelmann
  public Map<String, ClassCoverage> getClassCoverages() {
	  return classes;
  }
  ...
  // 08-05-2020 Added to avoid printing all statistics at the end while using delegating listener: Hendrik Winkelmann
  public boolean publishAtEnd =  true;
  @Override
  public void publishFinished(Publisher publisher) {
    ...
    if (publisher instanceof ConsolePublisher && publishAtEnd) {
